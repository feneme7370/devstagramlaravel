        ...
        <div class="md:w-1/2">
            ...
            @if ($post->user_id === auth()->user()->id)
                <form action="{{ route('posts.destroy', $post) }}" method="POST">
                    {{-- metodo spoofi, solo soporta el navegador GET y POST, pero esto utiliza otros --}}
                    @method('DELETE')
                    @csrf
                    <input class="bg-red-500 hover:bf-red-600 p-2 rounded text-white font-bold mt-4 cursor-pointer"
        ...
        <div class="md:w-1/2">
            <div class="shadow bg-white p-5 mb-5">
                @auth
                    ...
                    @if (session('mensaje'))
                        <div class="p-2 bg-green-500 rounded mb-6 text-white text-center uppercase font-bold">
                            {{ session('mensaje') }}
                        </div>
                    @endif
                    ...
                    <form action="{{ route('comentarios.store', ['post' => $post, 'user' => $user]) }}" method="POST">
                        ...
                        {{-- comentario --}}
                        <div class="mb-5">
                            <label class="mb-2 block uppercase text-gray-500 font-bold" for="comentario" id="">
                                Agregar comentario
                            </label>
                            <textarea
                                class="border p-3 w-full rounded-lg resize-none
                                    @error('name') border-red-500 @enderror" 
                                name="comentario" id="comentario" placeholder="Tu comentario" rows="3"
